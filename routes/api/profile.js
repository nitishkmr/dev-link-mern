...
router.get("/me", auth, async (req, res) => {
  try {
    const profile = await Profile.findOne({ user: req.user.id }) //  'user' is from the Schema from Profile model and corresponds to the ObjectID //  'req.user.id' is from the token being sent / auth on this route
...
router.post(
  ...
  [
    ...
    [
      ...
      check("status", "Status is required").not().isEmpty(), //is not empty
  ...
  async (req, res) => {
    ...
    if (youtube) {
      if (youtube.includes("http://") || youtube.includes("https://"))
        //append http for links as else the links will just append in the base url
